        ...
        try {
            if (req.readyState == 4 && req.status == 200) {
                ...
                $(rss).find("item").each(function (index, item) {
                    ...
                    '<div class="postThumb"><img style="width: 100%;height:208px;" src="' + post.getImage() + '"></div>' +
                    '<h1>' + post.title + '</h1>' +
...
var article = function (post, hashtable) {
    ...
    this.getImage = function () {
        var image = description.substr(0, description.indexOf('>') + 1);
        if (image.length == 0) {
    ...
    this.getDescription = function () {
        return description.substr(description.indexOf('>') + 1, 300) + "...";
    }
